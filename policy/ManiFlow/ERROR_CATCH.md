...
Update: 2025-08-05, thanks to Sergey for the contributions!
- numba version error: 
```bash
pip install numba==0.61.2
```

- no enough shared memory: add the following line to `RUN_DOCKER_CONTAINER.sh` and re-run it
```bash
--shm-size=8g \

# or consider using larger shared memory sizes based on your workload:

# Moderate (typical ML workloads)
--shm-size=16g

# Aggressive (large models, big datasets)
--shm-size=32g

# Maximum flexibility (use host's full shared memory)
--ipc=host
```
