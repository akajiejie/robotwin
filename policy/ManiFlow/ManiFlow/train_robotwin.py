...
##### One-time setup for torch.backends #####
##### Only for MeanFlow, not for ManiFlow #####
# print warnings#

disable_flash_mem_efficient_sdp = False
if disable_flash_mem_efficient_sdp:
    cprint("Disabling flash and memory efficient SDP for MeanFlow", 'red')
    import torch
...
from maniflow.env_runner.robotwin_runner import RobotRunner
...
class TrainManiFlowWorkspace:
    ...
    def run(self):
        ...
        use_dynamic_masking = cfg.task.dataset.get('use_dynamic_masking', True)
        cprint(f"Using dynamic masking: {use_dynamic_masking}", 'yellow')
        mask_scheduler = ModalityMaskScheduler(
            initial_img_ratio=cfg.task.dataset.get('initial_img_mask_ratio', 0.0),
            initial_point_ratio=cfg.task.dataset.get('initial_point_mask_ratio', 0.0),
            final_img_ratio=cfg.task.dataset.get('img_cond_mask_ratio', 0.0),
            final_point_ratio=cfg.task.dataset.get('point_cond_mask_ratio', 0.8),
            warmup_epochs=cfg.task.dataset.get('mask_warmup_epochs', 50),
            total_epochs=cfg.training.num_epochs,
            schedule_type=cfg.task.dataset.get('mask_schedule_type', 'linear')
        )
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                ...
                if use_dynamic_masking:
                    img_ratio, point_ratio = mask_scheduler.get_mask_ratios(self.epoch)
                    dataset.set_mask_ratios(img_ratio, point_ratio)
                    # Log current mask ratios
                    cprint(f"Epoch {self.epoch}: img_mask_ratio={dataset._current_img_mask_ratio:.3f}, point_mask_ratio={dataset._current_point_mask_ratio:.3f}", 'yellow')
            ...
            if (self.epoch % cfg.training.val_every) == 0 and RUN_VALIDATION:
                with torch.no_grad():
                    val_losses = list()
                    # asser len(val_dataloader) > 0, "Validation dataloader is empty"
                    with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                        ...
                        for batch_idx, batch in enumerate(tepoch):
                            ...
                            if cfg.policy.use_train_state:
                                ...
                                # Forward pass
                                loss, loss_dict = self.model.compute_loss(batch, train_state)
                            else:
                                # Forward pass
    ...
    def eval(self, mode='best'):
        ...
        all_rollout_steps = [10, 1] # [10, 1, 4, 2, 8]
    ...
    def get_policy_and_runner(self, cfg, checkpoint_num=3000):
        ...
        if not cfg.policy.use_pc_color:
            ckpt_file = pathlib.Path(f'./checkpoints/{self.cfg.task.name}/{checkpoint_num}.ckpt')
        else:
            ckpt_file = pathlib.Path(f'./checkpoints/{self.cfg.task.name}_w_rgb/{checkpoint_num}.ckpt')
    ...
    # Todo: remove this method, use get_policy_and_runner instead
    def get_robotwin_policy_and_runner(self, cfg, usr_args, mode='latest'):
        # load the latest checkpoint
...
class ModalityMaskScheduler:
    """Progressively increase missing modality ratio during training"""
    def __init__(self, 
                 initial_img_ratio=0.0,
                 initial_point_ratio=0.0,
                 final_img_ratio=0.3,
                 final_point_ratio=0.5,
                 warmup_epochs=50,
                 total_epochs=500,
                 schedule_type='linear'):
        self.initial_img_ratio = initial_img_ratio
        self.initial_point_ratio = initial_point_ratio
        self.final_img_ratio = final_img_ratio
        self.final_point_ratio = final_point_ratio
        self.warmup_epochs = warmup_epochs
        self.total_epochs = total_epochs
        self.schedule_type = schedule_type
    
    def get_mask_ratios(self, current_epoch):
        ...
        if self.schedule_type == 'linear':
            img_ratio = self.initial_img_ratio + (self.final_img_ratio - self.initial_img_ratio) * progress
            point_ratio = self.initial_point_ratio + (self.final_point_ratio - self.initial_point_ratio) * progress
        elif self.schedule_type == 'cosine':
            # Cosine annealing
            import math
            cosine_factor = 0.5 * (1 + math.cos(math.pi * (1 - progress)))
            img_ratio = self.final_img_ratio - (self.final_img_ratio - self.initial_img_ratio) * cosine_factor
            point_ratio = self.final_point_ratio - (self.final_point_ratio - self.initial_point_ratio) * cosine_factor
        elif self.schedule_type == 'exponential':
            # Exponential increase
            img_ratio = self.initial_img_ratio + (self.final_img_ratio - self.initial_img_ratio) * (1 - math.exp(-5 * progress))
            point_ratio = self.initial_point_ratio + (self.final_point_ratio - self.initial_point_ratio) * (1 - math.exp(-5 * progress))
        
        return img_ratio, point_ratio
