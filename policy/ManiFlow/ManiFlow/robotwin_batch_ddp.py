...
class DDPWorkspace:
    ...
    def run(self):
        ...
        if cfg.training.distributed:
            ...
            train_dataloader = create_ddp_dataloader(dataset, **cfg.dataloader)
        ...
        val_dataloader = create_ddp_dataloader(val_dataset, **cfg.val_dataloader)
                ...
                if is_main_process:
                    ...
                    with torch.no_grad():
                        ...
                        with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                            ...
                            for batch_idx, batch in enumerate(tepoch):
                                # batch = dict_apply(batch, lambda x: x.to(self.device, non_blocking=True))
                                batch = val_dataset.postprocess(batch, self.device)
                                if cfg.policy.use_train_state:
                                    # get train state
                                    train_state = {
                                        'cur_train_epoch': self.epoch,
...
class DistributedBatchSampler:
    """
    DDP-aware version of the custom BatchSampler that yields numpy arrays
    for the custom batching logic while properly distributing data across processes.
    """
    ...
    def __init__(
        ...
        # Calculate per-process data distribution
        self._calculate_distribution()
        
        # Each process gets its own RNG with different seed
        self.rng = np.random.default_rng(seed + self.rank) if shuffle else None

    def _calculate_distribution(self):
        """Calculate how to distribute data across processes."""
        # Total number of complete batches across all processes
        total_batches = self.data_size // self.batch_size
        
        if self.drop_last:
            # Drop incomplete batches
            usable_data_size = total_batches * self.batch_size
        else:
            usable_data_size = self.data_size
            
        # Distribute batches across processes
        batches_per_process = total_batches // self.world_size
        extra_batches = total_batches % self.world_size
        
        # Some processes get one extra batch if there are remainder batches
        if self.rank < extra_batches:
            self.num_batches_this_process = batches_per_process + 1
            self.start_batch_idx = self.rank * (batches_per_process + 1)
        else:
            self.num_batches_this_process = batches_per_process
            self.start_batch_idx = extra_batches * (batches_per_process + 1) + \
                                  (self.rank - extra_batches) * batches_per_process
        
        # Calculate actual data indices for this process
        self.start_data_idx = self.start_batch_idx * self.batch_size
        self.end_data_idx = min(
            (self.start_batch_idx + self.num_batches_this_process) * self.batch_size,
            usable_data_size
        )
        
        # Handle case where last process might have smaller batch if not drop_last
        if not self.drop_last and self.rank == self.world_size - 1:
            self.end_data_idx = self.data_size

    def __iter__(self):
        """Generate batches for this process."""
        # Create indices for this process's data portion
        if self.shuffle:
            # For reproducible distributed shuffling, all processes use same base permutation
            # but each takes different slice
            np.random.seed(self.seed)  # Same seed for all processes
            full_perm = np.random.permutation(self.data_size)
            process_indices = full_perm[self.start_data_idx:self.end_data_idx]
        else:
            process_indices = np.arange(self.start_data_idx, self.end_data_idx)
        
        # Create batches from this process's indices
        actual_data_size = len(process_indices)
        
        if self.drop_last:
            # Drop incomplete batch at the end
            num_complete_batches = actual_data_size // self.batch_size
            if num_complete_batches > 0:
                process_indices = process_indices[:num_complete_batches * self.batch_size]
                batched_indices = process_indices.reshape(-1, self.batch_size)
            ...
            if num_complete_batches > 0:
                complete_indices = process_indices[:num_complete_batches * self.batch_size]
                batched_indices = complete_indices.reshape(-1, self.batch_size)
                
                for batch_indices in batched_indices:
                    yield batch_indices
            
            # Yield incomplete batch if exists
            remaining = actual_data_size % self.batch_size
            if remaining > 0:
                incomplete_batch = process_indices[-remaining:]
                yield incomplete_batch

    def __len__(self):
        """Return number of batches this process will yield."""
        actual_data_size = self.end_data_idx - self.start_data_idx
        
        if self.drop_last:
            return actual_data_size // self.batch_size
        else:
            return (actual_data_size + self.batch_size - 1) // self.batch_size

    def set_epoch(self, epoch: int):
        """Set epoch for reproducible shuffling across epochs."""
        if self.shuffle:
            self.seed = self.seed + epoch * 1000  # Change seed each epoch
...
def create_ddp_dataloader(
    ...
    *,
    ...
    """Create DDP-aware dataloader with custom batching logic."""
    ...
    # def collate(x):
    #     """Custom collate function for numpy array batching."""
    #     assert len(x) == 1, f"Expected single batch, got {len(x)}"
    #     return x[0]
    ...
    dataloader = torch.utils.data.DataLoader(
        ...
        pin_memory=False,  # Handled by dataset
