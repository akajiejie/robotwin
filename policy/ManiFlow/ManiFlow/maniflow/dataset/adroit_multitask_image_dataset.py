...
ADROIT_ACTION_LENGTH = 28


class AdroitMultitaskImageDataset(BaseDataset):
    def __init__(self,
            data_path: str,
            task_names: List[str],
            horizon=1,
        ...
        for task_name in task_names:
            zarr_path = Path(data_path) / f"adroit_{task_name}_expert.zarr"
    ...
    def get_normalizer(self, mode='limits', **kwargs):
        ...
        for task_name in self.task_names:
            ...
            # Padding action to ADROIT_ACTION_LENGTH
            if rb['action'].shape[1] < ADROIT_ACTION_LENGTH:
                padding = np.zeros((rb['action'].shape[0], ADROIT_ACTION_LENGTH - rb['action'].shape[1]))
                all_actions.append(np.concatenate([rb['action'], padding], axis=1))
            else:
                all_actions.append(rb['action'])
            all_agent_pos.append(rb['state'][...,:])
            all_images.append(rb['img'])
        
        data = {
            'action': np.concatenate(all_actions),
            # 'agent_pos': np.concatenate(all_agent_pos),
            # 'image': np.concatenate(all_images),
        }
    ...
    def _sample_to_data(self, sample, task_name):
        ...
        # padding action to ADROIT_ACTION_LENGTH
        if sample['action'].shape[1] < ADROIT_ACTION_LENGTH:
            padding = np.zeros((sample['action'].shape[0], ADROIT_ACTION_LENGTH - sample['action'].shape[1]))
            action = np.concatenate([sample['action'], padding], axis=1).astype(np.float32)
        else:
...
if __name__ == "__main__":
    ...
    task_names = ['door', 'pen', 'hammer']
    # task_names = ['pen', 'hammer']
    dataset = AdroitMultitaskImageDataset(
        data_path=data_path,
