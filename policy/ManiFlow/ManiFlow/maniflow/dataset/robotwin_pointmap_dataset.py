...
class RoboTwinDataset(BaseDataset):
    def __init__(self,
        ...
        self.n_obs_steps = n_obs_steps  # number of observation steps to return, the rest will be discarded
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        if self.use_point_map:
            if random.random() < point_mask_ratio:
                head_point_cloud_copy = torch_data['obs']['head_point_cloud'].clone()
                torch_data['obs']['head_point_cloud'] = torch.zeros_like(torch_data['obs']['head_point_cloud'])
                # set mask to 0
                torch_data['obs']['head_point_cloud_mask'] = torch.tensor([False], dtype=torch.bool)  # False means masked
            else:
                torch_data['obs']['head_point_cloud_mask'] = torch.tensor([True], dtype=torch.bool)   # True means not masked

        # if use both image and point cloud, and mask both, select only one modality to mask
        if self.use_img and self.use_point_map and self.no_overlap:
            if (not torch_data['obs']['head_cam_mask']) and (not torch_data['obs']['head_point_cloud_mask']):
                if random.random() < 0.5:
                    ...
                    torch_data['obs']['head_point_cloud_mask'] = torch.tensor([True], dtype=torch.bool)
                    torch_data['obs']['head_point_cloud'] = head_point_cloud_copy
