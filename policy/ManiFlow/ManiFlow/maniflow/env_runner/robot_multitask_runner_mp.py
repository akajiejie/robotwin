...
class RobotMultitaskRunner(BaseRunner):
    def __init__(self,
        ...
        self.original_devices = [device if 'cuda' in device else f"cuda:{device}" for device in self.devices]
        
        # Get both rank allocation and device mapping
        self.task_rank_allocation, self.rank_to_device = allocate_tasks_to_gpus(task_names, self.original_devices)
    ...
    def create_env_runner(self, task_name, **kwargs):
    ...
    def _run_single_task(self, task_name: str, policy: BasePolicy, 
        ...
        try:
            ...
            runner = self.create_env_runner(task_name)
            if runner is None:
    ...
    def run(self, policy: BasePolicy, save_video=True):
        # If only one process, run tasks sequentially without multiprocessing
        if self.max_processes <= 1:
            ...
            for task_idx, task_name in enumerate(self.task_names, 1):
                try:
                    # # Get GPU rank for this task
                    # rank = self.task_rank_allocation[task_name]
                    # device = self.rank_to_device[rank]
                    
                    cprint(f"\nRunning task {task_idx}/{total_tasks}: {task_name} on {policy.device}", "cyan")
                    ...
                    #         shutil.copy2(tmp_video_path, persistent_video_path)
                    #         task_results['sim_video_eval']._path = persistent_video_path
                    #         cprint(f"Video saved to: {persistent_video_path}", "cyan")
                    #     else:
                    #         cprint(f"Warning: Video not found at {tmp_video_path}", "yellow")
                    ...
                    cprint(f"Completed task: {task_name} with success rate: {success_rate:.2%}", "green")
        ...
        # Original multiprocessing code for max_processes > 1
        else:
            try:
