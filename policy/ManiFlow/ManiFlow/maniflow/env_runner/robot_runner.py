...
class RobotRunner(BaseRunner):
    ...
    def get_action(self, policy: BasePolicy, observaton=None) -> bool: # by tianxing chen
        ...
        with torch.no_grad():
            ...
            obs_dict_input['head_point_cloud_in_bound_indices'] = obs_dict['head_point_cloud_in_bound_indices'].unsqueeze(0)
    ...
    def get_seed_list(self):
        with open(f'{pathlib.Path(__file__).parent.parent}/config/robotwin_task_config/eval_seeds/'+self.task_conifg['task_name']+'.txt', 'r') as file:
    ...
    def run(self, policy: BasePolicy, save_video=True):
        ...
        for episode_idx in tqdm.tqdm(range(self.eval_episodes), desc=f"Eval in Robotwin {self.task_name} Pointcloud Env", leave=False, mininterval=self.tqdm_interval_sec):
            ...
            try:
                ...
                env.apply_dp3_online(policy, self, device=device)
            except:
                print(f"Error in {self.task_name} with seed {now_seed}")
                # try again
                for i in range(3):
                    try:
                        env.close()
                        ...
                        print(f"Got error: {e}")
                        print(f"Error in {self.task_name} with seed {now_seed} for {i} times")
                        pass
