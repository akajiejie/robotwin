...
class RobotRunner(BaseRunner):
    ...
    def get_action(self, policy: BasePolicy, observaton=None) -> bool: # by tianxing chen
        ...
        # device transfer and dtype conversion (only for floating point tensors)
        def to_device_dtype(x):
            tensor = torch.from_numpy(x).to(device=device)
            # Only convert dtype for floating point tensors
            if tensor.dtype.is_floating_point:
                tensor = tensor.to(dtype=dtype)
            return tensor
        obs_dict = dict_apply(np_obs_dict, to_device_dtype)
        # run policy
        with torch.no_grad():
            obs_dict_input = {}  # flush unused keys
            obs_dict_input['point_cloud'] = obs_dict['point_cloud'].unsqueeze(0)
            obs_dict_input['head_cam'] = obs_dict['head_cam'].unsqueeze(0)
            obs_dict_input['left_wrist_cam'] = obs_dict['left_wrist_cam'].unsqueeze(0)
            obs_dict_input['right_wrist_cam'] = obs_dict['right_wrist_cam'].unsqueeze(0)
