...
from maniflow.model.vision_2d.timm_pointmap_encoder import TimmPointmapEncoder
...
class ManiFlowUnifiedTransformerPointcloudPolicy(BasePolicy):
    def __init__(self, 
            ...
            pointcloud_encoder: TimmPointmapEncoder = None,
        ...
        cprint(f"[ManiFlowUnifiedTransformerPointcloudPolicy] use_point_map: {self.use_point_map}", "red")
        ...
        point_cloud_obs_feature_dim = pointcloud_encoder.output_shape()[-1]
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        """
        if self.use_point_map and 'head_point_cloud_mask' not in obs_dict:
            if self.mask_point_cloud_inference:
                # mask the point cloud in inference as zero
                obs_dict['head_point_cloud'] = torch.zeros_like(obs_dict['head_point_cloud'], device=self.device, dtype=obs_dict['head_point_cloud'].dtype)
                B = obs_dict['head_point_cloud'].shape[0]
                # create a mask for point cloud
                obs_dict['head_point_cloud_mask'] =  torch.zeros((B, 1), device=self.device, dtype=torch.bool)  # [B, 1]
            else:
                B = obs_dict['head_point_cloud'].shape[0]
                obs_dict['head_point_cloud_mask'] = torch.ones((B, 1), device=self.device, dtype=torch.bool)  # [B, 1]
        ...
        # if not self.use_pc_color and self.use_point_map:
        ...
        if self.use_point_map:
            # point map and robot state features
            this_nobs = dict_apply(nobs, lambda x: x[:,:self.n_obs_steps,...].to(self.device))
            point_map_features = self.pointcloud_encoder(this_nobs)
            point_cond = point_map_features.reshape(B, -1, self.point_cloud_obs_feature_dim) # B, T*L2, C

            base_point_mask = nobs['head_point_cloud_mask'].to(self.device)  # [B, 1]
        ...
        nsample = self.conditional_sample(
            ...
            point_mask=point_mask if self.use_point_map else None,
            **self.kwargs)
    ...
    def compute_loss(self, batch, train_state=None, **kwargs):
        ...
        if self.use_point_map:
            ...
            this_nobs = dict_apply(nobs, lambda x: x[:,:self.n_obs_steps,...].to(self.device))
            point_map_features = self.pointcloud_encoder(this_nobs)
            point_cond = point_map_features.reshape(batch_size, -1, self.point_cloud_obs_feature_dim) # B, T*L2, C
