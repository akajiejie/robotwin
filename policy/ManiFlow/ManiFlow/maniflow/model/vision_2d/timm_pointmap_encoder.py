...
def unwrap_model(model):
    """Unwrap DataParallel or DistributedDataParallel wrapper"""
    if isinstance(model, (nn.DataParallel, nn.parallel.DistributedDataParallel)):
        return model.module
    return model

class TimmPointmapEncoder(ModuleAttrMixin):
    def __init__(self,
            shape_meta: dict,
            model_name: str,
            pretrained: bool,
        ...
        if model_name == "r3m":
            ...
            # model.eval()
            if pretrained:
                model = unwrap_model(model) # unwrap DataParallel or DistributedDataParallel
        ...
        cprint(f"Using RGB for Pointmap Encoder: {use_rgb}", 'red')
        for key, attr in obs_shape_meta.items():
            shape = tuple(attr['shape'])
            type = attr.get('type', 'low_dim')
            key_shape_map[key] = shape
            if type == 'rgb' and use_rgb:
                rgb_keys.append(key)

                this_model = model if share_rgb_model else copy.deepcopy(model)
                # this_model = this_model.to(device)  # Ensure on correct device
                ...
                # no transform for point cloud
                transform = nn.Identity()
                cprint(f"Using identity transform for point cloud input {key}", 'yellow')
                self.register_buffer("min_pcds", torch.tensor([-1.0, -1.0, -1.0]).reshape(1, -1, 1, 1))
                self.register_buffer("max_pcds", torch.tensor([1.0, 1.0, 1.0]).reshape(1, -1, 1, 1))
        ...
        point_keys = sorted(point_keys)
        low_dim_keys = sorted(low_dim_keys)
        print('rgb keys:         ', rgb_keys)
        print('point_keys keys:  ', point_keys)
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            ...
            if img.max() > 1.0 and 'head_point_cloud' not in key:
            ...
            # # norm point cloud to [-1, 1]
            # if 'head_point_cloud' in key:
            #     pointmap = img.clone()
            #     pointmap = (pointmap - self.min_pcds) / (self.max_pcds - self.min_pcds) # (0, 1)
            #     pointmap = 2.0 * (pointmap - 0.5) # (-1.0, 1.0)
            #     img = pointmap
            ...
            #     ForkedPdb().set_trace()  # Start interactive debugging
            #     # print device info for debugging
            #     print(f"Error processing key {key} with model {self.key_model_map[key]}: {e}")
            #     print(f"Image shape: {img.shape}, Device: {img.device}, Model device: {self.key_model_map[key].device}")
            #     raise e
        ...
        for key in self.point_keys:
            pointmap = obs_dict[key]
            # normalize point cloud to [-1, 1]
            if pointmap.shape[-1] == 3:
                if len(pointmap.shape) == 5:
                    # B, T, H, W, C --> B, T, C, H, W
                    pointmap = pointmap.permute(0, 1, 4, 2, 3)
                elif len(pointmap.shape) == 4:
                    # B, H, W, C --> B, C, H, W
                    pointmap = pointmap.permute(0, 3, 1, 2)
            

            B, T = pointmap.shape[:2]
            assert B == batch_size
            pointmap = pointmap.reshape(B*T, *pointmap.shape[2:]) # B*T, C, H, W
            ...
            if pointmap.shape[2:] != self.key_shape_map[key]:
                target_H, target_W = self.key_shape_map[key][1], self.key_shape_map[key][2]
                # do torchvision resize
                # pointmap shape: Bx3xHxW
                # new size: Bx3xnHxnW
                pointmap = F.interpolate(pointmap, size=(target_H, target_W), mode='bilinear', align_corners=False)
            
            assert pointmap.shape[1:] == self.key_shape_map[key]
            pointmap = self.key_transform_map[key](pointmap).to(self.device)
            raw_feature = self.key_model_map[key](pointmap).to(self.device)
            feature = self.aggregate_feature(raw_feature)
            assert len(feature.shape) == 2 and feature.shape[0] == B * T
            features.append(feature.reshape(B, -1))
