...
class TransformerPointmapEncoder(ModuleAttrMixin):
    def __init__(self,
            shape_meta: dict,
    ...
    def forward(self, obs_dict):
        ...
        for key in self.rgb_keys:
            if key not in obs_dict:
                # cprint(f"Key {key} not found in obs_dict, skipping...", 'yellow')
                continue    
            img = obs_dict[key]
        ...
        for key in self.point_keys:
            ...
            assert B == batch_size
            pointmap = pointmap.reshape(B*T, *pointmap.shape[2:])
