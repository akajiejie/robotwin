...
class LocalSpatialEncoding(nn.Module):
    def __init__(self, d, num_neighbors, device, bn=False):
...
class LocalFeatureAggregation(nn.Module):
    def __init__(self, d_in, d_out, num_neighbors, device, bn=False):
        ...
        self.lse1 = LocalSpatialEncoding(d_out//2, num_neighbors, device, bn=bn)
        self.lse2 = LocalSpatialEncoding(d_out//2, num_neighbors, device, bn=bn)
...
class RandLANetEncoder(nn.Module):
    def __init__(self, d_in, num_neighbors=16, decimation=2, num_layers=4, device=torch.device('cpu')):
        ...
        for i in range(min(num_layers, len(layer_dims)-1)):
            ...
            encoder_layers.append(
                LocalFeatureAggregation(d_in_layer, d_out_layer, num_neighbors, device)
            )
