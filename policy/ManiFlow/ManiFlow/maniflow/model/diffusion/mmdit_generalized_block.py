...
from maniflow.model.diffusion.mmdit_block import (
    JointAttention
)
...
class MMDiTBlock_Generalized(Module):
    def __init__(
        self,
        *,
...
class MMDiT(Module):
    def __init__(
        ...
        **block_kwargs
        ...
        blocks = [MMDiTBlock_Generalized(dim_modalities = dim_modalities, num_residual_streams = num_residual_streams, **block_kwargs) for _ in range(depth)]
...
if __name__ == '__main__':
    ...
    modality_tokens += (text_tokens,)

    video_tokens = torch.randn(2, 1024, 512)

    modality_tokens += (video_tokens,)

    audio_tokens = torch.randn(2, 256, 384)

    modality_tokens += (audio_tokens,)
    ...
    modality_masks = (
        text_mask,  # Text mask
        None,  # Video mask (not used)
        # None  # Audio mask (not used)
    )
    ...
    import pdb; pdb.set_trace()
    text_tokens, video_tokens, audio_tokens = output
    print('text tokens:', text_tokens.shape)
    print('video tokens:', video_tokens.shape)
    print('audio tokens:', audio_tokens.shape)