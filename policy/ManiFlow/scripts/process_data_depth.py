...
def load_hdf5(dataset_path):
    ...
    with h5py.File(dataset_path, "r") as root:
        ...
        for cam_name in root[f"/observation/"].keys():
            ...
            camera_config_dict[cam_name] = dict()
            camera_config_dict[cam_name]['intrinsic_cv'] = root[f"/observation/{cam_name}/intrinsic_cv"][()]
            camera_config_dict[cam_name]['cam2world_gl'] = root[f"/observation/{cam_name}/cam2world_gl"][()]
        pointcloud = root["/pointcloud"][()]

    return left_gripper, left_arm, right_gripper, right_arm, vector, pointcloud, image_dict, depth_dict, camera_config_dict
...
def main():
    ...
    head_intrinsic_cv_arrays = []
    head_cam2world_gl_arrays = []
    ...
    while current_ep < num:
        ...
        for j in range(0, left_gripper_all.shape[0]):
            ...
            head_depth = depth_dict_all["head_camera"][j] / 1000.0  # convert to meters
            head_intrinsic_cv = camera_config_dict_all["head_camera"]['intrinsic_cv'][j]
            head_cam2world_gl = camera_config_dict_all["head_camera"]['cam2world_gl'][j]
            ...
            if j != left_gripper_all.shape[0] - 1:
                ...
                # Resize RGB images to 224x224
                head_img = cv2.resize(head_img, (target_size[1], target_size[0]))  # Note: cv2.resize uses (width, height)
                ...
                head_intrinsic_cv_arrays.append(head_intrinsic_cv)
                head_cam2world_gl_arrays.append(head_cam2world_gl)
    ...
    try:
        ...
        head_intrinsic_cv_arrays = np.array(head_intrinsic_cv_arrays)
        head_cam2world_gl_arrays = np.array(head_cam2world_gl_arrays)
        ...
        head_intrinsic_cv_chunk_size = (100, *head_intrinsic_cv_arrays.shape[1:])
        head_cam2world_gl_chunk_size = (100, *head_cam2world_gl_arrays.shape[1:])
        ...
        zarr_data.create_dataset(
            "head_intrinsic_cv",
            data=head_intrinsic_cv_arrays,
            chunks=head_intrinsic_cv_chunk_size,
            dtype="float32",
            overwrite=True,
            compressor=compressor,
        )
        zarr_data.create_dataset(
            "head_cam2world_gl",
            data=head_cam2world_gl_arrays,
            chunks=head_cam2world_gl_chunk_size,
            dtype="float32",
            overwrite=True,
            compressor=compressor,
        )
